...
-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSS
...
if(_parsedString){
[_parsedString appendString:[string stringByTrimmingCharactersInSet:[NSCharac
}
}
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSStr
if ([elementName isEqualToString:@"trkpt"]){
...
CLLocation *loc = [[CLLocation alloc] initWithLatitude:lat lo
double prevLat = [_lastProcessingElement[@"lat"] doubleValue]
double prevLon = [_lastProcessingElement[@"lon"] doubleValue]
CLLocation *prevLoc = [[CLLocation alloc] initWithLatitude:pr
CLLocationDistance prevDistance = [loc distanceFromLocation:p
_lastEpoch = _lastEpoch + (prevDistance / self.averageSpeed);
}else{
_lastEpoch = [NSDate date].timeIntervalSince1970;
}
_processingElement[@"time"] = [NSString stringWithFormat:@"%.15f", _l
}
...
_lastEpoch = [[NSISO8601DateFormatter new] dateFromString:_parsedString].time
...
CLLocation *prevLoc = [[CLLocation alloc] initWithLatitude:prevLat lo
...
CLLocation *nextLoc = [[CLLocation alloc] initWithLatitude:nextLat lo
CLLocationDistance nextDistance = [loc distanceFromLocation:nextLoc];
nextSpeed = nextDistance / (nextTime - time);
avg++;
double y = (sin(DEG2RAD(nextLon) - DEG2RAD(lon))) * cos(DEG2RAD(nextL
double x = (cos(DEG2RAD(lat)) * sin(DEG2RAD(nextLat))) - (sin(DEG2RAD
course = fmod(RAD2DEG(atan2(y, x)) + 360, 360.0);
lastCourse = course;
}
speed = self.averageSpeed > 0 ? self.averageSpeed : (prevSpeed + nextSpeed) /
if (i == totalTracksCount - 1){
course = lastCourse;
}
//+[NSKeyedArchiver archivedDataWithRootObject:requiringSecureCoding:error:]
//Reference: https://bottleofcode.com/posts/custom-gps-data-in-the-ios-simula
NSString *archivedLoc = archivedCLLocBase.copy;
archivedLoc = [archivedLoc stringByReplacingOccurrencesOfString:@"ALTPLACE" w
archivedLoc = [archivedLoc stringByReplacingOccurrencesOfString:@"LATPLACE" w
archivedLoc = [archivedLoc stringByReplacingOccurrencesOfString:@"LONGPLACE"
archivedLoc = [archivedLoc stringByReplacingOccurrencesOfString:@"COURSEPLACE
archivedLoc = [archivedLoc stringByReplacingOccurrencesOfString:@"HORZACPLACE
archivedLoc = [archivedLoc stringByReplacingOccurrencesOfString:@"LIFESPAPLAC
archivedLoc = [archivedLoc stringByReplacingOccurrencesOfString:@"SPEEDPLACE"
archivedLoc = [archivedLoc stringByReplacingOccurrencesOfString:@"TIMESTAMPPL
archivedLoc = [archivedLoc stringByReplacingOccurrencesOfString:@"TYPEPLACE"
archivedLoc = [archivedLoc stringByReplacingOccurrencesOfString:@"VERTACPLACE
